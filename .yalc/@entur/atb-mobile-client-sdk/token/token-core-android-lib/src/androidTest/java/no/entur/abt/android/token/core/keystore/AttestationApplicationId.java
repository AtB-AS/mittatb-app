...
import static no.entur.abt.android.token.core.keystore.AndroidKeyAttestationConstants.ATTESTATION_APPLICATION_ID_PACKAGE_INFOS_INDEX;
import static no.entur.abt.android.token.core.keystore.AndroidKeyAttestationConstants.ATTESTATION_APPLICATION_ID_SIGNATURE_DIGESTS_INDEX;
import static no.entur.abt.android.token.core.keystore.AndroidKeyAttestationConstants.ATTESTATION_PACKAGE_INFO_PACKAGE_NAME_INDEX;
import static no.entur.abt.android.token.core.keystore.AndroidKeyAttestationConstants.ATTESTATION_PACKAGE_INFO_VERSION_INDEX;
 ...
 * 
 * This data structure reflects the Android platform's belief as to which apps are allowed to use the secret key material under attestation. The ID can comprise
 * multiple packages if and only if multiple packages share the same UID.
 */
public class AttestationApplicationId implements Comparable<AttestationApplicationId> {
	...
	private AttestationApplicationId(DEROctetString attestationApplicationId) throws IOException {
		ASN1Sequence attestationApplicationIdSequence = (ASN1Sequence) ASN1Primitive.fromByteArray(attestationApplicationId.getOctets());
