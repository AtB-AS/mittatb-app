...
import {Networking as RCTNetworking} from 'react-native';
...
class EventSource extends EventTarget(...EVENT_SOURCE_EVENTS) {
   ...
   */
  constructor(url: string, eventSourceInitDict?: any | null | undefined) {
    super();
    ...
    this._subscriptions.push(
      RCTNetworking.addListener('didReceiveNetworkResponse', (args: ArgType) =>
    ...
    this._subscriptions.push(
      RCTNetworking.addListener(
        ...
        (args: ArgType) => this.__didReceiveIncrementalData(...args),
      ),
    );
    this._subscriptions.push(
      RCTNetworking.addListener('didCompleteNetworkResponse', (args: ArgType) =>
  ...
  __didReceiveIncrementalData(requestId?: number, responseText?: string) {
    if (requestId !== this._requestId) {
      return;
    }

    if (responseText) this.__processEventStreamChunk(responseText);
  }

  __didCompleteResponse(requestId?: number, error?: string): void {
    if (requestId !== this._requestId) {
      return;
    }
