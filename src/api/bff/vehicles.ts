...
import {VehicleFragment} from '../types/generated/fragments/vehicles';
...
export const getServiceJourneyVehicles = async (
  ...
  const result = await client.get<VehicleFragment[]>(stringifyUrl(url, query), {
    ...opts,
  });
...
export const useLiveVehicleSubscription = ({
  ...
  vehicle: VehicleFragment | undefined;
  enabled: boolean;

  connectionIndicatorDebounceTimeInMs?: number;
}) => {
  const [liveVehicle, setLiveVehicle] = useState<VehicleFragment | undefined>(
    vehicle,
  );
  ...
  const onMessageHandler = useCallback(
    (event: WebSocketMessageEvent) => {
      ...
      const vehicle = JSON.parse(event.data) as VehicleFragment;
